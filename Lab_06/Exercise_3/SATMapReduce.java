...
public class SATMapReduce {
    ...
    public static void main(String[] args) {
        ...
        List<String>[] threadOutputs = new List[numThreads];
        ...
        for (int i = 0; i < numThreads; i++) {
            threadOutputs[i] = new LinkedList<>();
            int from = i * blockSize;
            int to = (i == numThreads - 1) ? iterations : from + blockSize;

            threads[i] = new Thread(new Worker(from, to, size, threadOutputs[i]));
            threads[i].start();
        }
        ...
        for (List<String> partial : threadOutputs) {
            finalOutput.addAll(partial);
        }
    ...
    static class Worker implements Runnable {
        ...
        public Worker(int from, int to, int size, List<String> localList) {
            this.from = from;
            this.to = to;
            this.size = size;
