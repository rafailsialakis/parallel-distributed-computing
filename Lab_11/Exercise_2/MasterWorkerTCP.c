...
int main(int argc, char* argv[])
{
    ...
    for (current_workers = 0; current_workers < workers; current_workers++) {
        ...
        handler_args = malloc(sizeof(struct WorkerArgs)); 
        handler_args->start = current_workers * block;
        handler_args->end = (current_workers + 1) * block;
        handler_args->h = h;
        handler_args->client_socket = connfd;
        //printf("start %d end %d h %.10lf\n", handler_args->start, handler_args->end, handler_args->h);
        pthread_create(&threadIDs[current_workers], NULL, connection_handler, handler_args);
